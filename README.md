...
Memory Compass 采用创新的**三层混合记忆系统**，为 AI Agent 提供企业级的数据持久化和容灾能力。

**核心优势：**
- ☁️ LanceDB: 高速语义搜索 + 跨会话记忆
- 💾 文件系统：确定性和可读性 + WAL 协议
- 🛡️ Checkpoints: 防御性备份 + 任务续传

详细技术架构请查看：[**🧠 完整架构文档**](ARCHITECTURE.md)
...
└──────────────┬──────────────────────────┘
    ...
    ┌──────────┼──────────┐
    ↓          ↓          ↓
[LanceDB] [SESSION-STATE] [Checkpoints]
 (O(log n))  (实时固化)   (15min/45min)
```
